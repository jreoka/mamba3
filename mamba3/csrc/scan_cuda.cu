...
template <typename scalar_t, int N, bool SaveCheckpoints>
...
#pragma unroll
  ...
  for (int chunk = num_checkpoints - 1; chunk >= 0; --chunk) {
    ...
    // Restore the exact state at the beginning of this chunk, then replay its
    // short forward recurrence into shared memory. Layout [time, state, thread]
    // keeps loads/stores coalesced across neighboring channels.
#pragma unroll
    for (int n = 0; n < N; ++n) {
      if (n < d_state) {
...
void launch_forward(
    ...
    int channels,
    int channel_blocks) {
#define LAUNCH_FWD(N) scan_forward_kernel<scalar_t, N, SaveCheckpoints> \
...
void launch_backward(
    ...
    int channels,
    int channel_blocks) {
#define LAUNCH_BWD(N, STRIDE) scan_backward_kernel<scalar_t, N> \
  ...
  if (d_state <= 8) LAUNCH_BWD(8, 16);
  else if (d_state <= 16) LAUNCH_BWD(16, 8);
  else if (d_state <= 32) LAUNCH_BWD(32, 4);
  else LAUNCH_BWD(64, 2);
#undef LAUNCH_BWD
}
      ...
      [&] {
        if (save_states) {
          launch_forward<scalar_t, true>(
              ...
              batch, length, channels, channel_blocks);
        } else {
